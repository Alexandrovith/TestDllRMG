...
//			return;
	...
	Mess->Out ("Создан [%s][%s]", asParamName.c_str (), requestName.c_str ());//device->getName (), 
...
RESP_RETVAL RmgTransaction::processResponse (DWORD size)
{
	...
	while (1)
	{
		...
		if (membIt != memb.end ())
		{
			...
			{
				if (StateWr == StateWR::SendHeadOK)									// Если был получен ответ на запись данных ФХП
				{
#ifdef _DEBUG
...
#endif // DEBUG
				// Если пришло сообщение об ошибке записи от прибора
				if (Dev->getStorage ()->getConnectionStatus ())
				{
					StateWr = StateWR::NoSendYet;
					//Создать R1 и W0 для повторной записи 
					//Transaction *Tr;
