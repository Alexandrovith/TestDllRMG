...
bool CComPort::Opening()
{
	...
	if (iRet != 0)
	{
		Mess->Save (MessErr(iRet)); return false;//LOGF(((SerialDev*)Own)->getName(), MessErr(iRet))
	}
	...
	Mess->Save ("Порт COM%d открыт", iPort);		// LOGF (((SerialDev*)Own)->getName (), "Порт COM%d открыт", iPort);
	return true;
}
...
bool CComPort::ClosePort()
{
	if (bPortIsOpen)
	{
		...
		try
		{
			...
			sio_close(iComPort);
			Mess->Save ("Порт COM%d закрыт", iComPort);		// LOGF (((SerialDev*)Own)->getName (), "Порт COM%d закрыт", iComPort);
		}
		catch (...) {}
	}
	return bPortIsOpen;
...
bool CComPort::Send(char *cpBufTX, int iSize)
{
	if (bPortIsOpen)
	{
		...
		if (iRet < 0)
		{
			Mess->Save ("COM%d: %s", iComPort, MessErr(iRet));		// LOGF(((SerialDev*)Own)->getName(), "COM%d: %s", iComPort, MessErr(iRet));
			return false;
...
int CComPort::Recieve(char* cpBufRX, int iMaxSizeRX, int& iCountBites, std::string asEnd)
{
	...
	if (iCountBites < 0)
	{
		Mess->Save ("COM%d: %s", iComPort, MessErr(iCountBites));// LOGF(((SerialDev*)Own)->getName(), 
		return 1;
	}
...
const char* CComPort::MessErr(int err)
{
	...
	case SIO_WIN32FAIL: 		ShowSysErr(); return (const char*)lpMsgBuf;   //return "Запрос функции Win32 потерпел неудачу.";
...
//_____________________________________________________________________________
//Запуск сканирования прихода сообщения о посылке
bool MessIsReady()
{
	return false;
}
