...
bool CComPort::Connecting()
{
	...
	Mess->Out ("Порт COM%d открыт", iPort);		// LOGF (((SerialDev*)Own)->getName (), "Порт COM%d открыт", iPort);
	return true;
}
...
{
	if (bConnected)								// Если порт был открыт -
		Disconnect();								//  закрываем оный
	bConnected = Connecting();		// Открываем порт 
	return bConnected;
}
...
bool CComPort::Disconnect()
{
	if (bConnected)
	{
		...
		try
		{
			...
			sio_close(iComPort);
			Mess->Out ("Порт COM%d закрыт", iComPort);		// LOGF (((SerialDev*)Own)->getName (), "Порт COM%d закрыт", iComPort);
		}
		catch (...) {}
	}
	return bConnected;
